...
public class POJO extends BaseEntity {
	...
	public boolean delete() {
		boolean dr = (QueryHelper.update("DELETE FROM " + getTableName()
				+ " WHERE id=?", getId()) == 1);
	...
	public <T extends POJO> T get(long id) {
		...
		return (T) QueryHelper.readFromCache(getClass(),
				isCachedByID() ? getCacheRegion() : null, id, sql, id);
	}
	...
	public boolean updateAttr(String attrName, Object attrValue) {
		String sql = "UPDATE " + getTableName() + " SET " + attrName
				+ " = ? WHERE id = ?";
	...
	public long totalCount() {
		if (isCachedByID())
			return (long) QueryHelper.statFromCache(getCacheRegion(),
					OBJ_COUNT_CACHE_KEY, "SELECT COUNT(*) FROM "
							+ getTableName());
		return (long) QueryHelper
				.stat("SELECT COUNT(*) FROM " + getTableName());
	}
	...
	public List<Long> getKeys(String key, String filter, Object... params) {
		String sql = "SELECT " + key + " FROM " + getTableName() + " WHERE "
				+ filter;
		...
		return QueryHelper.queryFromCache(Long.class, getQueryCacheRegion(),
				cacheKey, sql, params);
	}
