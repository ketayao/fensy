...
public class POJO extends BaseEntity {
	...
	public boolean delete() {
		boolean dr = (QueryHelper.update("DELETE FROM " + getTableName() + " WHERE id=?", getId()) == 1);
	...
	public <T extends POJO> T get(long id) {
		...
		return (T) QueryHelper.readFromCache(getClass(), isCachedByID() ? getCacheRegion() : null, id, sql, id);
	}
	...
	public boolean updateAttrs(String[] attrNames, Object[] attrValues) {
		...
		String sql = "UPDATE " + getTableName() + " SET " + StringUtils.join(kvs, ',') + " WHERE id = ?";
	...
	public long totalCount(String filter) {
		return (long) QueryHelper.stat("SELECT COUNT(*) FROM " + getTableName() + " WHERE " + filter);
	}
